...
public class ThreeDEngine extends ACSGameEngine {
  ...
  protected ThreeDEngine(int screenWidth, int screenHeight, int pixelWidth, int pixelHeight, boolean fullScreen, boolean useRetina) {
  ...
  public boolean onUserCreate() {
    ...
    //projection matrix
    matProjection = Matrix4x4.projection(90.0f, (float) screenHeight() / (float) screenWidth(), 0.1f, 1000.0f);
  ...
  public boolean onUserUpdate(float elapsedTime) {
    ...
    if (getKey(Key.S).held) {
      vCamera = subtactVector(vCamera, forward);
    }

    if (getKey(Key.A).held) {
      yaw -= 2.0f * elapsedTime;
    }

    if (getKey(Key.D).held) {
      yaw += 2.0f * elapsedTime;
    }
    ...
    for (Triangle tri : meshCube.tris) {
      ...
      if (vectorDotProduct(normal, cameraRay) < 0) {
        ...
        for (int n = 0; n < clippedTriangles; n++) {
          ...
          triProjected.points[0].x *= 0.5f * screenWidth();
          triProjected.points[0].y *= 0.5f * screenHeight();
          triProjected.points[1].x *= 0.5f * screenWidth();
          triProjected.points[1].y *= 0.5f * screenHeight();
          triProjected.points[2].x *= 0.5f * screenWidth();
          triProjected.points[2].y *= 0.5f * screenHeight();
    ...
    for (Triangle triToRaster : trianglesToRaster) {
      ...
      for (int p = 0; p < 4; p++) {
        ...
        while(newTriangles > 0){
          ...
          switch (p)
          {
            case 0:	trisToAdd = Triangle_ClipAgainstPlane(new  Vector3D(0.0f, 0.0f, 0.0f), new Vector3D(0.0f, 1.0f, 0.0f), test, clipped[0], clipped[1]); break;
            case 1:	trisToAdd = Triangle_ClipAgainstPlane(new Vector3D(0.0f, (float)screenHeight() - 1, 0.0f ), new Vector3D(0.0f, -1.0f, 0.0f ), test, clipped[0], clipped[1]); break;
            case 2:	trisToAdd = Triangle_ClipAgainstPlane(new Vector3D(0.0f, 0.0f, 0.0f ), new Vector3D(1.0f, 0.0f, 0.0f ), test, clipped[0], clipped[1]); break;
            case 3:	trisToAdd = Triangle_ClipAgainstPlane(new Vector3D((float)screenWidth() - 1, 0.0f, 0.0f ), new Vector3D(-1.0f, 0.0f, 0.0f ), test, clipped[0], clipped[1]); break;
          }
...
//    v.w = input.x * matrix.m[0][3] + input.y * matrix.m[1][3] + input.z * matrix.m[2][3] + matrix.m[3][3];
  ...
  private int Triangle_ClipAgainstPlane(Vector3D plane_p, Vector3D plane_n, Triangle in_tri, Triangle out_tri1, Triangle out_tri2) {
    ...
    if (insidePointCount == 2 && outsidePointCount == 1) {
      out_tri1.pixel = Pixel.GREEN;
      out_tri2.pixel = Pixel.RED;
  ...
  public static void main(String[] args) {
    int pixelDim = 2;
    ThreeDEngine acsGameEngine = new ThreeDEngine(256, 240, pixelDim, pixelDim, false, true);
