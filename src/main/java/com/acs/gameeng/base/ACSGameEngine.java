...
//    private boolean[] keyOldState = new boolean[300];
    ...
    public abstract boolean onUserUpdate(double fElapsedTime);

    public abstract boolean onUserDestroy();

    protected ACSGameEngine(int screenWidth, int screenHeight, int pixelWidth, int pixelHeight, boolean fullScreen, boolean useRetina) {
    ...
    private void loop() {
        ...
        while (!glfwWindowShouldClose(window) && atomActive) {
            ...
            while (!glfwWindowShouldClose(window) && atomActive) {
                ...
                if (!onUserUpdate(elapsedTime / 1000.0))
                    atomActive = false;
...
//    return keyboardState[key.getValue()];
    ...
    public void drawTriangle(Triangle triangle, Pixel pixel){
